...
/// <summary>
/// Represents a media source that loads media from an absolute URI.
/// </summary>
public sealed partial class UriEpubSource : EpubSource
{
	/// <summary>
	/// Bindable property for the <see cref="Uri"/> property.
	/// </summary>
	public static readonly BindableProperty UriProperty =
		BindableProperty.Create(nameof(Uri), typeof(Uri), typeof(UriEpubSource), propertyChanged: OnUriSourceChanged, validateValue: UriValueValidator);

	/// <summary>
	/// Initializes <see cref="UriEpubSource"/>
	/// </summary>
	public UriEpubSource()
	{
	...
	/// <summary>
	/// An implicit operator to convert a string value into a <see cref="UriEpubSource"/>.
	/// </summary>
	/// <param name="uri">The URI string to convert into a <see cref="UriEpubSource"/>.</param>
	public static implicit operator UriEpubSource(string uri) => (UriEpubSource)FromUri(uri);

	/// <summary>
	/// An implicit operator to convert a <see cref="UriEpubSource"/> into a string value.
	/// </summary>
	/// <param name="uriEpubSource">A <see cref="UriEpubSource"/> instance to convert to a string value.</param>
	public static implicit operator string?(UriEpubSource? uriEpubSource) => uriEpubSource?.Uri?.ToString();
	...
	static void OnUriSourceChanged(BindableObject bindable, object oldValue, object newValue) =>
		((UriEpubSource)bindable).OnSourceChanged();
